...
nonisolated struct MemberCenterListItem: Sendable, Equatable, Identifiable {
    ...
    init(movie: MediaSummaryDTO, destination: MemberCenterDestination) {
    ...
    init(series: MediaSummaryDTO, destination: MemberCenterDestination) {
        self.id = "\(destination.rawValue)-tv-\(series.id)"
        self.title = series.title
        self.subtitle = BaseDisplayTextFormatter.announcedText(series.releaseDate)
